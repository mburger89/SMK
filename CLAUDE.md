...
| `DefaultKeymapGenerated.swift` | **GENERATED — do not edit.** `defaultKeymapBytes: [UInt8]` for this board, pre-encoded in the version-2 binary payload format (`KeymapBinary.swift`'s `decodeKeymapPayload`) — a `#if` chain over the `SMK_BOARD_*` flags, one payload per board, `smk_kbd` as the `#else`. Produced by `./generate_default_keymap.sh` from `boards/*.json` (plus `keymap.json` for the three boards using `layersFrom`) and `keycodes.json` (HID usages come from the same manifest `generate_keycodes.sh` reads — not hardcoded). **The same run also rewrites `Tests/SMKCoreTests/BoardPayloadsGenerated.swift`** — commit both or the round-trip test runs against a stale fixture. `KeymapBinaryTests`' `compiledDefaultKeymapRoundTrips` pins the generator against the decoder |
...
Every board's keymap — its GPIO matrix *and* its layers — lives in `boards/<name>.json` and is compiled at build time by `./generate_default_keymap.sh` into `Sources/SMKCore/DefaultKeymapGenerated.swift`'s `defaultKeymapBytes`. Edit the board file (or, for the three boards using `layersFrom`, `keymap.json`'s `layers` — its `matrix` object is read by nothing and editing it is a silent no-op) and re-run the script, then commit **both** regenerated files (`DefaultKeymapGenerated.swift` and `Tests/SMKCoreTests/BoardPayloadsGenerated.swift`) — the same generated-file pattern as `KeyCodesGenerated.swift`. `Main.swift` gets the layers via `LayerEngine.loadKeymap(binary:count:)` and the matrix via `Config(payload:)`, both from that one payload, so the two cannot disagree about how many rows and columns a board has.
...
The runtime keymap store and the compiled-in default both hold a **version-2 binary payload**, not JSON. Full design rationale (the JSON-vs-binary size measurement, the decision to compile the whole keymap rather than enlarge storage, etc.) is in `docs/superpowers/specs/2026-08-21-binary-keymap-format-design.md` — what follows is the byte contract itself, reproduced in full rather than summarized, since a paraphrase is exactly what drifts out of sync silently. The configurator now has its own compiler for this format (`~/esp/smk_configurator/Sources/SMKConfigurator/Model/KeymapCompiler.swift`), and that repo's `CLAUDE.md` documents the contract from its side too. Neither copy is generated from the other, so treat both as hand-maintained prose that can drift: what actually pins the two implementations together is the configurator's `BinaryFormatAgreementTests`, which compiles `keymap.json` and compares the bytes against this repo's generated `DefaultKeymapGenerated.swift`. That test guards the **bytes**, not the documentation — a change to this table still has to be made in both files by hand.
...
**Four implementations of this format now exist, and nothing but tests keeps them agreeing** — a change to the tag table, the header layout, or the macro-step layout has to land in all four:
- `Sources/SMKCore/KeymapBinary.swift` — the decoder (`decodeKeymapPayload`)
- `generate_default_keymap.sh` — the build-time generator of every board's compiled-in payload
- `Tests/SMKCoreTests/PayloadBuilder.swift` — the **test-only** encoder that keeps the host tests readable. Pinned byte-for-byte against the generator by `builderMatchesShellGenerator`, which is the only thing stopping it drifting
- the configurator's compiler (`KeymapDocument` → binary payload) — lives in `~/esp/smk_configurator`
