 ...
 * subsys/bluetooth/controller/hci_internal.c, pinned commit
 * f48ab6dfbddeb240a21cda86109480d0e2e12f57. This project has no Zephyr host
 * on the other side of this dispatcher (BTstack fills that role instead),
 * so the three Zephyr header dependencies below have been replaced with
 * sdc_bt_compat.h, a from-scratch shim exposing exactly the symbols this
 * file needs (see that file's header comment for the full, verified list
 * and why it's bigger than a first grep pass suggested).
 * "hci_internal_wrappers.h" is likewise vendored under
 * sdc_hci_dispatch_wrappers.h. DO NOT re-sync this file against a newer
 * Zephyr-coupled upstream without redoing this adaptation — see Task 6 of
...
#include "sdc_hci_dispatch_wrappers.h"
